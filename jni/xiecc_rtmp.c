...
int rtmp_open_for_write(const char *url, uint32_t video_width,
	...
	if (RTMP_IsConnected(rtmp)) {
		...
		*output++ = AMF_ECMA_ARRAY;
		...
		output = AMF_EncodeNamedNumber(output, outend, &av_height,
				video_height);
...
//	if (audio_config_ok == false) {
	if (size == 2) {
		...
		body_len = 2 + size; //aac header + raw data size // adts_len - AAC_ADTS_HEADER_SIZE; // audito tag header + adts_len - remove adts header + AudioTagHeader
		output_len = body_len + FLV_TAG_HEAD_LEN + FLV_PRE_TAG_LEN;
		output = malloc(output_len);
		// flv tag header
 ...
 */
static uint8_t * get_nal(uint32_t *len, uint8_t **offset, uint8_t *start,
		uint32_t total) {
	uint32_t info;
	uint8_t *q;
	uint8_t *p = *offset;
...
// @param [in] abs_ts     : indicate whether you'd like to use absolute time stamp
int rtmp_sender_write_video_frame(uint8_t *data, int size, uint64_t dts_us,
		int key, uint32_t abs_ts) {
	uint8_t * buf;
	uint8_t * buf_offset;
	int val = 0;
	int total;
