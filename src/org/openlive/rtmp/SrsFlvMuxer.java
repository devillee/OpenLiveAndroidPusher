...
/**
 * Created by winlin on 5/2/15. Updated by leoma on 4/1/16. to POST the
 * h.264/avc annexb frame to SRS over RTMP.
 * 
 * @see android.media.MediaMuxer
 *      https://developer.android.com/reference/android/media/MediaMuxer.html
 *
 *      Usage: muxer = new SrsRtmp("rtmp://ossrs.net/live/yasea");
 *      muxer.start();
 *
 *      MediaFormat aformat =
 *      MediaFormat.createAudioFormat(MediaFormat.MIMETYPE_AUDIO_AAC,
 *      asample_rate, achannel); // setup the aformat for audio. atrack =
 *      muxer.addTrack(aformat);
 *
 *      MediaFormat vformat =
 *      MediaFormat.createVideoFormat(MediaFormat.MIMETYPE_VIDEO_AVC,
 *      vsize.width, vsize.height); // setup the vformat for video. vtrack =
 *      muxer.addTrack(vformat);
 *
 *      // encode the video frame from camera by h.264 codec to es and bi, //
 *      where es is the h.264 ES(element stream). ByteBuffer es,
 *      MediaCodec.BufferInfo bi; muxer.writeSampleData(vtrack, es, bi);
 *
 *      // encode the audio frame from microphone by aac codec to es and bi, //
 *      where es is the aac ES(element stream). ByteBuffer es,
 *      MediaCodec.BufferInfo bi; muxer.writeSampleData(atrack, es, bi);
 *
...
public class SrsFlvMuxer {
			...
			public void run() {
				...
				while (!Thread.interrupted()) {
					...
					synchronized (txFrameLock) {
						try {
							// isEmpty() may take some time, so we set timeout
	...
	public void writeSampleData(int trackIndex, ByteBuffer byteBuf, MediaCodec.BufferInfo bufferInfo) {
		if (bufferInfo.offset > 0) {
			Log.w(TAG, String.format("encoded frame %dB, offset=%d pts=%dms", bufferInfo.size, bufferInfo.offset,
	...
	/**
	 * the aac object type, for RTMP sequence header for
	 * AudioSpecificConfig, @see aac-mp4a-format-ISO_IEC_14496-3+2001.pdf, page
	 * 33 for audioObjectType, @see aac-mp4a-format-ISO_IEC_14496-3+2001.pdf,
	 * page 23
	 */
	...
	/**
	 * the FLV/RTMP supported audio sample rate. Sampling rate. The following
	 * values are defined: 0 = 5.5 kHz = 5512 Hz 1 = 11 kHz = 11025 Hz 2 = 22
	 * kHz = 22050 Hz 3 = 44 kHz = 44100 Hz
	 */
	...
	private class SrsRawH264Stream {
		...
		public SrsFlvFrameBytes mux_avc2flv(ArrayList<SrsFlvFrameBytes> frames, int frame_type, int avc_packet_type,
				int dts, int pts) {
		...
		public void writeVideoSample(final ByteBuffer bb, MediaCodec.BufferInfo bi) {
			...
			while (bb.position() < bi.size) {
				...
				if (nal_unit_type == SrsAvcNaluType.SPS || nal_unit_type == SrsAvcNaluType.PPS) {
					Log.i(TAG, String.format("annexb demux %dB, pts=%d, frame=%dB, nalu=%d", bi.size, pts, frame.size,
							nal_unit_type));
				}
